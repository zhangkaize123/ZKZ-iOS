...
-(void)loadNews
{
    ...
    NSURL *url=[NSURL URLWithString:@"http://127.0.0.1/news/findnews.php"];
    ...
    if (data!=nil) {
        ...
        if(array==nil){
            //提示
            UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"友情提示" message:@"没有此类的新闻" delegate:self cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil];
            [alert show];
        }
...
-(void)loadSearchNewsWith:(NSString *)str
{
    ...
    NSString *urlStr=[NSString stringWithFormat:@"http://127.0.0.1/news/findnews.php?"];
    NSLog(@"%@",urlStr);
    NSURL *url=[NSURL URLWithString:urlStr];
    NSMutableURLRequest *resquest=[NSMutableURLRequest requestWithURL:url];
    [resquest setHTTPMethod:@"post"];
    NSString *poststr=[NSString stringWithFormat:@"titlename=%@",str];
    NSData *postdata=[poststr dataUsingEncoding:NSUTF8StringEncoding];
    [resquest setHTTPBody:postdata];
    NSData *data=[NSURLConnection sendSynchronousRequest:resquest returningResponse:nil error:nil];
...
-(void)loadlocationNewsWith:(NSString *)str
{
    ...
    FMResultSet *rs=[db executeQuery:@"select * from xinwenpaixu where title like ?",str];
    if ([rs next]) {
        while ([rs next]) {
            ZKZNews *news=[[ZKZNews alloc]init];
            news.title=[rs stringForColumn:@"title"];
   ...
   NSString *str=searchBar.text;
    ...
    if(str.length>0)//有搜索内容
    {
        ...
        if (status==NotReachable) {//没有网络
            UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"友情提示" message:@"对不起没有网络！！" delegate:self cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil];
            [alert show];
