...
-(void)saveData:(UIBarButtonItem *)sender
{
    ...
    FMResultSet *rs=[db executeQuery:@"select * from savenews where idid=?",self.news.idid];
    ...
    BOOL b=[ db executeUpdate:@"insert into savenews(idid,title,subtitle,picture,content,author,flid,click,time,pic) values(?,?,?,?,?,?,?,?,?,?)",self.news.idid,self.news.title,self.news.subtitle,self.news.picture,self.news.content,self.news.author,self.news.flid,self.news.click,self.news.time,data];
    if (b) {
